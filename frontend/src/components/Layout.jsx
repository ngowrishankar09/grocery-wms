...
import {
  ...
  LayoutGrid, Table2, BookOpen, MapPin, ShoppingBag, Smartphone, Menu, Store, RotateCcw, Receipt,
  UserCircle2, LogOut, Shield, Tags
} from 'lucide-react'
...
function NotificationsBell({ align = 'right' }) {
      ...
      {open && (
        <div className={`absolute ${align === 'left' ? 'left-0' : 'right-0'} top-full mt-2 w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 flex flex-col max-h-[80vh]`}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <button onClick={load} className="text-xs text-gray-400 hover:text-gray-600 px-2 py-1 rounded hover:bg-gray-100">
                Refresh
              </button>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {alert.action && (
                          <span className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-full ${s.bg} ${s.text} font-medium`}>
                            → {alert.action}
                          </span>
                        )}
                      </div>
          ...
          {data && data.total > 0 && (
            <div className="px-4 py-2.5 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex items-center justify-between text-xs text-gray-500">
              ...
              <Link to="/reports" onClick={() => setOpen(false)} className="text-blue-500 hover:underline font-medium">
                Full report →
              </Link>
            </div>
          )}
...
// ── Main Layout ───────────────────────────────────────────────
// Pages that manage their own scroll/height (no p-6 wrapper)
...
function SidebarContent({ t, lang, setLang, onNavClick, user, logout }) {
  const allowed = user ? ROLE_NAV[user.role] : null  // null = show all
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className="hidden md:flex w-60 bg-gray-900 text-white flex-col flex-shrink-0">
        <div className="p-4 border-b border-gray-700 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-white">🏪 Grocery WMS</h1>
            <p className="text-xs text-gray-400 mt-0.5">Warehouse Manager</p>
          </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-w-0">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-gray-900 text-white flex-shrink-0">
          ...
          <span className="font-bold text-sm">🏪 Grocery WMS</span>
          <NotificationsBell />
        </header>
